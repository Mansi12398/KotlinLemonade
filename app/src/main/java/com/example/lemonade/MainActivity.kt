...
class MainActivity : AppCompatActivity() {
    ...
    private fun clickLemonImage() {
     ...
     /*   when(lemonadeState){
        ...
        if(lemonadeState == SELECT){
            ...
            Log.w(TAG, "clickLemonImage: ${lemonSize}", )
            squeezeCount = 0
            lemonImage?.setImageResource(R.drawable.lemon_squeeze)
        }else if(lemonadeState == SQUEEZE){
            totalCount.setText(lemonSize.toString())
            squeezeCount++
            lemonSize--
            Log.w(TAG, "clickLemonImage: squeezeCount: $squeezeCount :: lemonsize : $lemonSize" )
    ...
    private fun setViewElements() {
        ...
        Log.w(TAG, "Lemon state :txt: $lemonadeState")
